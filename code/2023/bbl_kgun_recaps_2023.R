...
all_birds <- rbind(n2023,a2023,m2023)

###Read in recap files
nrecap <- read_excel("data/2023/kgun_recap_NOPI_2023.xlsx")
head(nrecap)
mrecap <- read_excel("data/2023/kgun_recap_MALL_2023.xlsx")

###Combine recap data
all_recaps <- rbind(nrecap,mrecap)
...
all_recaps$band_number <- paste(all_recaps$PREFIX,all_recaps$SUFFIX,sep = "-")
...
novel_recaps <- all_recaps[!all_recaps$band_number %in% all_birds$band_number,]
...
write.xlsx(kgun_recaps_bbl, "output/2023/kgun_recaps_bbl.xlsx")
